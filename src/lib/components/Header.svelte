  ...
  <div class="flex flex-row justify-between">
    ...
    <!-- Logo link -->
      <a sveltekit:prefetch href="/" class="mr-6">
        <svg class="h-12 w-12 flex-shrink-0 {$darkMode ? 'text-white' : 'text-slate-500'} transition-colors duration-150 ease-in-out" id="logo" fill="none" viewBox="0 0 115 36" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="pt-3 flex flex-row space-x-1">
      ...
      <button type="button" class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring focus:ring-sky-300 dark:focus:ring-yellow-400 {$darkMode ? 'bg-slate-700' : 'bg-slate-200'}" aria-pressed="false"
