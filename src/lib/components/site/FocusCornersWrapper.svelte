<script lang="ts">
  ...
  export let twTopLeftCornerColor: string = 'text-foreground';
  export let twBottomLeftCornerColor: string = 'text-foreground';
  export let twTopRightCornerColor: string = 'text-foreground';
  export let twBottomRightCornerColor: string = 'text-foreground';
  /*
...
<div class={cn("relative group overflow-hidden", className, animation)} {...$$restProps}>
  ...
  <Icons.chevronRight class="absolute z-10 w-14 h-14 rotate-45 bottom-0 right-0 md:group-hover:bottom-1 md:group-hover:right-1 lg:group-hover:bottom-2 lg:group-hover:right-2 transition-all duration-500 {twBottomRightCornerColor}" />
